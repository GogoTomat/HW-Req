...
class YDiskUploader:
    ...
    def upload(self):
        ...
        if not os.path.isfile(self.file_path):
             result["error"] = True
             result["msg"] = 'Incorrect path to file:', {self.file_path}
             return result
        ...
        good_codes = (200, 201)
        if not response.status_code in good_codes:
             resp_info = json.loads(response.text)
             result["error"] = True
        ...
        if not response.status_code in good_codes:
            ...
            result["error"] = True
            result["msg"] = "Error with attempt to upload the file to YDisk:", {error_info['message']}
            return result
